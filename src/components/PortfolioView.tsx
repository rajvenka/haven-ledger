...
interface PortfolioViewProps {
  ...
  portfolioBankBalanceHistory?: any[];
  ...
  setProjectedBankBalance?: (amount: number, portfolioId?: string, notes?: string) => Promise<void>;
...
export default function PortfolioView(props: PortfolioViewProps) {
  ...
  // #40 fix: when several portfolios are selected together, check whether they all share
  // one currency before falling back to the workspace base - previously only an exact
  // single-portfolio selection was recognized, so selecting e.g. ETORO RAJ + ETORO SASI
  // (both USD) silently fell back to INR even though both portfolios agree on USD.
  const selectedHeaderPortfoliosForCurrency = selectedHeaderPortfolioIds
    ? portfolios.filter((p: any) => selectedHeaderPortfolioIds.includes(p.id))
    : [];
  const sharedHeaderCurrency = selectedHeaderPortfoliosForCurrency.length > 0
    && selectedHeaderPortfoliosForCurrency.every((p: any) => p.currency === selectedHeaderPortfoliosForCurrency[0].currency)
    ? selectedHeaderPortfoliosForCurrency[0].currency
    : null;
  const headerDisplayCurrency = singleHeaderPortfolio ? singleHeaderPortfolio.currency : (sharedHeaderCurrency || baseCurrency);
            ...
            <div className="apple-card p-4 space-y-2">
              ...
              {editingProjectedBankBalance ? (
                <div className="space-y-2">
                  ...
                  <input
                    type="text"
                    value={projectedBankBalanceNotesInput}
                    onChange={(e) => setProjectedBankBalanceNotesInput(e.target.value)}
                  ...
                  <div className="flex gap-1.5">
                    <button
                      onClick={() => runAction(async () => {
                        await setProjectedBankBalance?.(parseFloat(projectedBankBalanceAmountInput) || 0, projPortfolioId, projectedBankBalanceNotesInput);
                        setEditingProjectedBankBalance(false);
                        setProjectedBankBalanceNotesInput('');
                      })}
                      className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase rounded-lg cursor-pointer"
                    >
                      Save
              ...
              {(() => {
                const bbHistory = portfolioBankBalanceHistory.filter((h: any) => (h.portfolio_id ?? null) === (projPortfolioId ?? null));
      ...
      <div className="space-y-4">
          ...
          {isImporting && (
            <div className="apple-card p-4 space-y-3">
              ...
              {/* Path chooser: Sync OR pick a file template → upload */}
              {!importRawParsed && (() => {
                ...
                const allTemplates: { id: BrokerTemplate; label: string; hint: string }[] = [
                  { id: 'zerodha', label: 'Zerodha', hint: 'Stocks + MF XLSX from Console' },
                ...
                return (
                  <div className="space-y-3">
                    ...
                    <button
                      ...
                      onClick={() => setImportPath(importPath === 'sync' ? null : 'sync')}
                      className={`w-full text-left rounded-2xl border p-3 transition-all ${
                        linked.length === 0
                          ? 'border-slate-200 dark:border-slate-800 opacity-40 cursor-not-allowed'
                          : importPath === 'sync'
                            ? 'border-indigo-400 dark:border-indigo-600 bg-indigo-50/60 dark:bg-indigo-950/30 ring-2 ring-indigo-400/40'
                    ...
                    {importPath === 'sync' && linked.length > 0 && (
                    ...
                    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/40 dark:bg-slate-900/40 p-3 space-y-2.5">
                      ...
                      <p className="text-[9px] text-slate-500">Choose the format that matches your download</p>
                      <div className="inline-flex flex-wrap items-center gap-0.5 p-0.5 rounded-full bg-violet-100/80 dark:bg-violet-950/50 border border-violet-200/60 dark:border-violet-900/60">
                        {allTemplates.map((tpl) => (
                          <button
                            ...
                            onClick={() => {
                              setImportPath('file');
                              setImportTemplate(tpl.id);
                              setImportPreview(null);
                              setImportRawParsed(null);
                            }}
                            title={tpl.hint}
                            className={`shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold transition-all ${
                              fileOpen && importTemplate === tpl.id
                                ? 'bg-violet-600 text-white shadow-md shadow-violet-600/30'
                                : 'text-violet-700/80 dark:text-violet-300/80 hover:text-violet-950 dark:hover:text-violet-100'
                            }`}
                          >
                            {tpl.label}
                          </button>
                      ...
                      {fileOpen && (
                        <div className="space-y-2 pt-1">
                          <p className="text-[9px] text-slate-400">
                            {importTemplate === 'zerodha' && 'Console → Holdings → Download as XLSX (stocks + MF detected automatically)'}
