    ...
    <div className="w-full min-w-0 max-w-full overflow-x-hidden space-y-3 box-border isolate">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 min-w-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h3 className="text-sm font-black text-slate-900 dark:text-white shrink-0">P&L Calendar</h3>
        </div>
      ...
      {/* Book — full width, horizontal scroll only */}
      {portfolios.length > 0 && onPortfolioChange && (
        <div className="w-full min-w-0 max-w-full">
          <div className="flex items-stretch gap-2 min-w-0">
            <span className="shrink-0 self-center text-[9px] font-black uppercase tracking-widest text-violet-500/80 w-9">
              Book
            </span>
            <div
            ...
            >
              <div className="flex items-center gap-1 pr-2 w-max">
                <button
                  ...
                  className={`shrink-0 px-2.5 py-1.5 rounded-full text-[10px] font-bold border ${
                    selectedPortfolioId === 'all'
                ...
                {portfolios.map((p) => (
                  <button
                    ...
                    className={`shrink-0 px-2.5 py-1.5 rounded-full text-[10px] font-bold border max-w-[7.5rem] truncate ${
      ...
      {/* Ccy — full width scroll */}
      {currenciesForBook.length > 0 && (
        <div className="w-full min-w-0 max-w-full">
          <div className="flex items-stretch gap-2 min-w-0">
            <span className="shrink-0 self-center text-[9px] font-black uppercase tracking-widest text-emerald-500/80 w-9">
              Ccy
            </span>
            <div
              className="flex-1 min-w-0 overflow-x-auto overscroll-x-contain touch-pan-x"
      ...
      {/* Data status banner */}
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/60 px-3 py-2 text-[10px] text-slate-500 space-y-0.5 min-w-0">
        <p className="break-words">
          <span className="font-bold text-slate-700 dark:text-slate-200">{filteredRows.length}</span> rows ·{' '}
        ...
        {distinctDates.length === 0 && (
          <p>
            No snapshots yet. Tap <span className="font-bold">Snapshot</span> after prices are fresh, or wait for market-close cron.
          </p>
        )}
        {distinctDates.length === 1 && (
          <p>
            Only <span className="font-bold">1 day</span> captured. Day P&L needs a second day. Today shows portfolio value (baseline).
          </p>
        )}
        {snapError && <p className="text-rose-600 font-bold break-words">Snapshot failed: {snapError}</p>}
      </div>

      <div className="flex items-center justify-between min-w-0">
        <button type="button" onClick={() => shift(-1)} className="p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 shrink-0">
      ...
      {loading ? (
        ...
        <div className="w-full min-w-0 max-w-full rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden bg-white dark:bg-slate-900">
          <div className="grid grid-cols-7 border-b border-slate-100 dark:border-slate-800">
            {['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'].map((d) => (
              <div key={d} className="px-0.5 py-1.5 text-center text-[9px] font-black uppercase text-slate-400">
                {d}
              </div>
            ))}
          </div>
            ...
            {monthCells.map((c, i) => {
              if (!c.date) {
                return (
                  <div
                    key={`e-${i}`}
                    className="min-h-[52px] sm:min-h-[64px] border-t border-r border-slate-50 dark:border-slate-800/50 bg-slate-50/40 dark:bg-slate-950/30"
                  />
                );
              }
              ...
              return (
                <button
                  ...
                  className={`min-h-[52px] sm:min-h-[64px] min-w-0 border-t border-r border-slate-50 dark:border-slate-800/50 p-1 text-left transition-colors overflow-hidden ${
                    selected
                      ? 'bg-violet-50 dark:bg-violet-950/40 ring-2 ring-inset ring-violet-500'
                      : agg
                ...
                >
                  ...
                  {agg && pnl != null ? (
                    <p
                      className={`text-[9px] sm:text-[11px] font-black tabular-nums mt-0.5 leading-tight break-all ${
                    ...
                    <p className="text-[8px] sm:text-[9px] text-slate-400 mt-0.5 leading-tight break-all">
                      {money(agg.marketValue, selectedCurrency)}
                      <span className="block text-[8px] opacity-70">base</span>
                    </p>
                  ) : (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 min-w-0">
          {yearMonths.map((ym) => (
            <button
              ...
              className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 text-left hover:ring-2 hover:ring-violet-400/40 min-w-0"
            >
              <p className="text-[11px] font-black text-slate-700 dark:text-slate-200">{ym.label}</p>
              <p
                className={`text-[13px] font-black tabular-nums mt-1 break-all ${
                  ym.count === 0 ? 'text-slate-300' : !ym.hasPnl ? 'text-slate-500' : ym.sum >= 0 ? 'text-emerald-600' : 'text-rose-600'
                }`}
              >
                {ym.count === 0 ? '-' : ym.hasPnl ? money(ym.sum, selectedCurrency) : `${ym.count}d`}
              </p>
      ...
      {drill && (
        <div className="rounded-2xl border border-violet-200 dark:border-violet-900/50 bg-violet-50/40 dark:bg-violet-950/20 p-3 space-y-2 min-w-0 max-w-full">
          <div className="flex items-center justify-between gap-2 min-w-0">
            <div className="min-w-0">
              <p className="text-[11px] font-black text-slate-900 dark:text-white">{drill.date}</p>
              {drill.dayPnl != null ? (
                <p
                  className={`text-sm font-black tabular-nums break-all ${
                ...
                <p className="text-sm font-black tabular-nums text-slate-600 dark:text-slate-300 break-all">
                  {moneyFine(drill.marketValue, drill.currency)}{' '}
                  <span className="text-[10px] font-bold text-slate-400">value (baseline)</span>
                </p>
              )}
            </div>
            <button type="button" onClick={() => setSelectedDay(null)} className="text-[10px] font-bold text-slate-400 shrink-0">
              Close
            </button>
          </div>
          <ul className="divide-y divide-slate-200/60 dark:divide-slate-800 max-h-56 overflow-y-auto">
            {[...drill.symbols]
              ...
              .map((s, i) => (
                <li key={`${s.symbol}-${i}`} className="py-1.5 flex items-center justify-between gap-2 text-[11px] min-w-0">
                  <div className="min-w-0">
                    <p className="font-bold text-slate-800 dark:text-slate-100 truncate">{s.symbol}</p>
                    {s.broker && <p className="text-[9px] text-slate-400 truncate">{s.broker}</p>}
                  </div>
                  <p
